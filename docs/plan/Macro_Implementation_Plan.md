...
### 5. ID 直通检索与类脑衰变引爆 (Phase 3)
原有的 `Retriever.ts` 主要是对 Embedding 向量和 Rerank 模型服务，我们需要增加基于 ID 直接命中的 `agenticSearch(recallIds: string[])` 直通管道：
1. **数据库底层提货**：拿着解析好的短 UUID 数组，直接向 `events` 表发出精确的 bulkGet 请求（极大缩减响应耗时和算力）。
2. **构建虚假打分交由类脑**：为命中的记录，直接套用预处理大模型给它钦定的 `score`，将它们包装为传统的 `RecallCandidate` 塞入 `BrainRecallCache.process(candidates)`！
3. **完美联动 DecayBomb**：当剧情发生大转折时，Agentic 裁判会自然抛弃上一个场景的全部激活记忆，只保留新召回的设定。此时交入缓存处理的 `candidates` 与原先的短期记忆（STM）交集为 0，这会**原汁原味地无缝触发由 `shouldTriggerDecayBomb` 控制的遗忘炸弹**，将衰草自动清理出局！
