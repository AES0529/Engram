...
### 2.6 Batch Engine (批处理引擎)
- **位置**: `src/modules/batch/engine/BatchEngine.ts`
- **职责**: 负责长程与高并发的后台批量处理。
- **机制**: 与关注流程编排的 Workflow Engine 拆隔离，专注于大吞吐量任务链的切片 (Generators)、进度节流通知与局部错误宽容能力，分发至 `TaskHandler` 执行具体业务。
...
### 3.4 典型流程：RAG 检索 (Retrieval Pipeline)
1. **Trigger**: 各种 `agenticSearch` 或 `hybridSearch` 进入 `Retriever`。
2. **Facade**: Retriever 调用 `WorkflowEngine.run(retrievalWorkflow, input)`。
3. **Steps**:
